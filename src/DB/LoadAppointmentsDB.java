 ...
 *
 */
public class LoadAppointmentsDB 
{
	/**
	...
	public static AppointmentList loadList(String username)
	{
		...
		ArrayList<String> response = DBInteraction.communicate(name,DBInteraction.connect(ActionURLs.LOAD_URL));
		...
		if(!response.get(0).equals("empty"))
		{
			try
			{
				...
				for(int i = 0; i < appJsonList.length(); i++)
				{
					...
					list.addAppointment(new Appointment(tempdesc, tempplace, tempdate.substring(0, 4), tempdate.substring(4,6), tempdate.substring(6,8), temptime.substring(0, 2), temptime.substring(2,4)), list.getRoot());
				}
			}
